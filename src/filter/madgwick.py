...
class MadgwickFilter(Filter):
    ...
    def update(self, a: Vector3, g: Vector3, m: Vector3):
        ...
        s0 = (-(2 * q.y) * (2.0 * (q.x * q.z) - (2 * q.w * q.y) - a.x) + (
                    2 * q.x) * (2.0 * (q.w * q.x) + (
                    2 * q.y * q.z) - a.y) - h.z * q.y * (
                          b * (0.5 - (q.y * q.y) - (q.z * q.z)) + h.z * (
                              (q.x * q.z) - (q.w * q.y)) - m.x) + (
                          -b * q.z + h.z * q.x) * (
                          b * ((q.x * q.y) - (q.w * q.z)) + h.z * (
                              (q.w * q.x) + (q.y * q.z)) - m.y) + b * q.y * (
                          b * ((q.w * q.y) + (q.x * q.z)) + h.z * (
                              0.5 - (q.x * q.x) - (q.y * q.y)) - m.z))
        s1 = ((2 * q.z) * (2.0 * (q.x * q.z) - (2 * q.w * q.y) - a.x) + (
                    2 * q.w) * (2.0 * (q.w * q.x) + (
                    2 * q.y * q.z) - a.y) - 4.0 * q.x * (
                          1 - 2.0 * (q.x * q.x) - 2.0 * (
                              q.y * q.y) - a.z) + h.z * q.z * (
                          b * (0.5 - (q.y * q.y) - (q.z * q.z)) + h.z * (
                              (q.x * q.z) - (q.w * q.y)) - m.x) + (
                          ...
                          b * ((q.w * q.y) + (q.x * q.z)) + h.z * (
                              0.5 - (q.x * q.x) - (q.y * q.y)) - m.z))
        s2 = (-(2 * q.w) * (2.0 * (q.x * q.z) - (2 * q.w * q.y) - a.x) + (
                    2 * q.z) * (2.0 * (q.w * q.x) + (
                    2 * q.y * q.z) - a.y) - 4.0 * q.y * (
                          1 - 2.0 * (q.x * q.x) - 2.0 * (q.y * q.y) - a.z) + (
                          -(2 * b) * q.y - h.z * q.w) * (
                          b * (0.5 - (q.y * q.y) - (q.z * q.z)) + h.z * (
                              (q.x * q.z) - (q.w * q.y)) - m.x) + (
                          b * q.x + h.z * q.z) * (
                          ...
                          b * ((q.w * q.y) + (q.x * q.z)) + h.z * (
                              0.5 - (q.x * q.x) - (q.y * q.y)) - m.z))
        s3 = ((2 * q.x) * (2.0 * (q.x * q.z) - (2 * q.w * q.y) - a.x) + (
                    2 * q.y) * (2.0 * (q.w * q.x) + (2 * q.y * q.z) - a.y) + (
                          -(2 * b) * q.z + h.z * q.x) * (
                          b * (0.5 - (q.y * q.y) - (q.z * q.z)) + h.z * (
                              (q.x * q.z) - (q.w * q.y)) - m.x) + (
                          -b * q.w + h.z * q.y) * (
                          b * ((q.x * q.y) - (q.w * q.z)) + h.z * (
                              (q.w * q.x) + (q.y * q.z)) - m.y) + b * q.x * (
                          b * ((q.w * q.y) + (q.x * q.z)) + h.z * (
                              0.5 - (q.x * q.x) - (q.y * q.y)) - m.z))
